...
2. Si la connexion personnelle manque, guider uniquement le parcours de l'application utilisée avec le [guide de connexion Mati](https://github.com/Mati-Technologies/mati-plugins/blob/main/docs/installation.md). Ce guide public reste consultable sans Brain. Claude Chat/Cowork utilise son gestionnaire de connecteurs; Claude Code propose l'authentification dans `/mcp`; Codex propose l'authentification dans ses réglages MCP. Hermes utilise le paquet portable décrit dans ce guide; vérifier la prise en charge OAuth de sa version avant de lancer une connexion, car certaines versions ne rendent pas les MCP portables accessibles à `hermes mcp login`. Un paquet chargé ne prouve pas que Brain est connecté. Les intitulés et noms de serveurs peuvent varier : suivre ce qui est réellement affiché. L'import GitHub de ce plugin ne couvre pas ChatGPT web/mobile.
